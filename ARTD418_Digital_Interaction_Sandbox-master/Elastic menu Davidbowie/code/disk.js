class Disk {
	...
	show() {
		...
		if (this.album != undefined) {
			this.p5.fill(253, 158, 130); // between 0 - 255 RGB color
			this.p5.noStroke();
	...
	textBox(w, h, side) {
		...
		if (this.mouseVerticalProximity() < 32) {
			...
			if (this.clicked) {
				...
				this.p5.text(this.description, xCoord + padding, 20 + ((this.pictureSize * 1.4) * this.scale + 25) + this.pos.y);
